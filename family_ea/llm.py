...
todos (things to do, with a deadline day or none) and reminders (a message to send someone
at a given moment); plus `today`, a member's «на сьогодні» board: free text, replaced
whole, only when asked.
"""
...
class LlmResult(BaseModel):
    ...
    photo: str = Field(default="", description="Опис фото для сторінки «Документи»; '' без фото")
...
людина розповідає, — це journal, items, events, todos і reminders; а «Списки на \
...
(items: що у нас є і де лежить), events, todos, reminders і список на сьогодні \
(today) кожного; щоранку о 08:30 писати кожному дайджест (списки на сьогодні, події на \
сьогодні й завтра, задачі на сьогодні, прострочені; задачі без дати лише на вебі); \
...
повідомлення». Підпис каже, що зробити: «зроби з цього таску» — todo, «додай у \
...
бути самодостатнім: фото лишається на вебі («Документи»), але в наступних повідомленнях \
ти його вже не побачиш. У reply не описуй фото, а підтверди, що записав: опис іде в photo. \
...
- photo — лише коли є фото: опис знімка для сторінки «Документи», де фото переглядають і \
шукають. Два-три речення: що це (чек, афіша, скріншот допису, коробка з книгами, документ), \
звідки чи від кого, і головне з нього як є: назви, суми, дати, номери, імена, адреси. \
Незалежно від того, чи щось записав і чи був підпис. Без фото — порожній рядок.
...
або null, якщо всієї сім'ї. Подія без дати — не подія: це нотатка або todo. Минулі \
...
відкритих todos, нотаток чи речей і поверни update / cancel / close / delete / remove. \
Не створюй дублікат.
