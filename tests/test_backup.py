...
def test_archive_holds_the_snapshot_and_the_notes(db: Database, family: Family) -> None:
    _seed(db)
    name, data = build_archive(db, family, KYIV, now=datetime(2026, 9, 11, 3, 30, tzinfo=KYIV))
    assert name == "family-2026-09-11.zip"
...
def test_backup_command_writes_the_archive(tmp_path: Path, capsys) -> None:
    ...
    _seed(src)
    src.close()
    path = backup(_settings(), tmp_path / "src.db", tmp_path / "backups")
